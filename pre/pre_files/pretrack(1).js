var campaignID = "69056c187ed98ead18a38759";
...
var initialSrc = "https://trk.vitariseboost.com/"+campaignID+"?format=json";
...
setTimeout(function(){
	if (!urlParams.get('rtkcid')) {
		...
		xhr.onreadystatechange = function() {
			if (xhr.readyState == 4 && xhr.status == 200) {
				...
				document.querySelectorAll('a').forEach(function(el) {
					if (el.href.indexOf("trk.vitariseboost.com/preclick")>-1) {
				...
				xhrr.open("GET", "https://trk.vitariseboost.com/preview?clickid="+rawData.clickid)
				xhrr.send();
			}
		}
		...
		xhrTrack.open("GET", "https://trk.vitariseboost.com/preview?clickid="+rtkClickID )
		xhrTrack.send();
		document.querySelectorAll('a').forEach(function(el) {
			if (el.href.indexOf("trk.vitariseboost.com/preclick")>-1) {
				if (el.href.indexOf('?')>-1) {
